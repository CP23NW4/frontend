...
const reqAdoption = async () => {
  try {
    ...
    if (res.status === 200 || res.status === 201) {
      ...
      alert("User can requesting to adopt a stray animal can submit only one request per post");
    }else {
      console.error("Error:", res.status, res.statusText);
    }
  } catch (error) {
...
onMounted(() => {
  ...
  <template>
  <div class="text-zinc-900 min-h-screen mt-20 flex items-center justify-center w-96">
    <div class="max-w-xl w-full p-8 bg-white shadow-lg rounded-lg">
      ...
      <form @submit.prevent="submitForm">
        ...
        <div class="grid grid-cols-1 gap-4">
          ...
          <!-- First Name -->
          <div>
            <label for="Name" class="block mb-1 text-left text-[14px]"
              >Name</label
            >
            <input
              type="text"
              ...
              id="name"
              disabled
              class="w-full bg-gray-200 border border-gray-300 rounded-md px-4 py-2 focus:outline-none focus:border-blue-400"
            />
          </div>
          <!-- Address -->
          <div>
            <label for="address" class="block mb-1 text-left text-[14px]"
              >Address</label
            >
            <textarea
              id="address"
  ...
  {{
          ...
          <!-- Phone Number -->
          <div>
            <label for="phoneNumber" class="block mb-1 text-left text-[14px]"
              >Phone Number</label
            >
            <input
            ...
            <input
              v-else
              type="text"
              v-model="reqForm.reqPhone"
              id="phoneNumber"
              disabled
              class="w-full border border-gray-300 rounded-md px-4 py-2 focus:outline-none focus:border-blue-400"
            />
          </div>
          <div>
            <label for="contact" class="block mb-1 text-left text-[14px]"
              >Other Contacts (Line, etc.)</label
            >
            <input
              v-if="user.contact !== null"
              type="text"
              v-model="reqForm.contact"
              id="phoneNumber"
              class="w-full border border-gray-300 rounded-md px-4 py-2 focus:outline-none focus:border-blue-400"
            />
            <input
          ...
          <div class="grid gap-3 md:grid-cols-1">
            <label
              class="block text-sm font-medium text-gray-900 dark:text-white text-left text-[14px]"
              for="file_input"
              >Upload your home Picture</label
            >
            <input
          ...
          <!-- Note -->
          <div>
            <label for="salary" class="block mb- text-left text-[14px]"
              >Salary (baht) for month</label
            >
            <input
              v-model="reqForm.salary"
              type="number"
              id="salary"
              class="w-full border border-gray-300 rounded-md px-4 py-2 focus:outline-none focus:border-blue-400"
            />
          ...
          <div>
            <label for="note" class="block mb- text-left text-[14px]"
              >Note</label
            >
            <textarea
        ...
        <div class="mt-6">
          <button
            type="submit"
            class="w-full bg-orange-500 text-white font-bold py-2 px-4 rounded-md hover:bg-gray-600 focus:outline-none focus:bg-gray-600"
          >
            Submit
          </button>
        </div>
  ...
  <div class="text-left mb-10 mt-2">
    <button
      @click="goBack"
      class="text-gray-600 font-semibold px-4 rounded-md hover:text-gray-800 focus:outline-none"
    >
      Back
    </button>
  </div>
